...
export function LoadingSkeleton({
  ...
  if (variant === "card") {
    return (
      <div className={cn("rounded-xl border border-border bg-card p-6", className)}>
        ...
        <div className="mt-6 h-9 w-28 bg-muted rounded-md animate-pulse" />
      </div>
    );
  }
  ...
  if (variant === "list-item") {
    return (
      <div className={cn("flex items-center gap-4 p-4 border border-border rounded-xl bg-card", className)}>
  ...
  // default text
  return (
    <div className={cn("h-4 w-full bg-muted rounded animate-pulse", className)} />
  );
}
