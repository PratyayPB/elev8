...
describe("Profile to Resume Mapper", () => {
  ...
  it("maps full profile with skills and education", () => {
  ...
  it("mergeProfileIntoResumeArtifact preserves existing non-profile sections", () => {
    const emptyArtifact = profileToResumeArtifact(resumeId, null);
    ...
    assert.strictEqual(mergedArtifact.skills.length, 1);
