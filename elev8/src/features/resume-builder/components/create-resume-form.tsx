...
export function CreateResumeForm({}: CreateResumeFormProps) {
  ...
  const onSubmit = async (data: CreateResumeFormInput) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = errData.error || "Failed to create resume";
        
        if (response.status === 409 || errData.code === "RESUME_TITLE_EXISTS") {
          setFieldError("title", {
            type: "manual",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      {error && (
        <div className="p-4 rounded-xl bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400 text-sm font-medium border border-red-200 dark:border-red-900/50 flex items-start gap-2.5">
          <FileText className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
          <div>
            <p>{error}</p>
            <p className="text-xs text-red-600/80 dark:text-red-400/80 mt-1">
              Please enter a unique name for your resume (e.g., adding a company name or version suffix).
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-text-primary flex items-center justify-between">
          ...
          <span className="text-[11px] font-normal text-text-muted">Must be unique</span>
        </label>
        <input
          {...register("title")}
          type="text"
          placeholder="e.g. Software Engineer Resume - Google Application"
          className={`w-full px-4 py-2.5 rounded-xl bg-surface-muted border text-sm text-text-primary placeholder:text-text-muted focus:outline-none transition-colors ${
            errors.title
        ...
        {errors.title ? (
          ...
          <p className="text-[11px] text-text-muted mt-1">
            Choose a distinct title to easily identify this resume in your workspace.
          </p>
        )}
      </div>
