...
export default function ProgressError({ error, reset }: ProgressErrorProps) {
  useEffect(() => {
    console.error("[ProgressError] Error boundary caught error:", error);
  }, [error]);
  ...
  const displayMessage = isAuthError
    ? "You do not have permission to view this career progress page."
    : error.message ||
      "An unexpected error occurred while loading your progress dashboard. Please try again.";
      ...
      <div className="w-full max-w-md bg-dashboard-card border border-dashboard-cardBorder rounded-[var(--card-radius-lg)] p-8 shadow-sm space-y-5">
        ...
        <div>
          <h2 className="text-xl font-display font-bold text-text-primary">
            {isAuthError ? "Access Denied" : "Unable to Load Progress"}
          </h2>
