...
export default function RoadmapViewerError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("[RoadmapViewerError] Error boundary caught error:", error);
  }, [error]);
  ...
  const displayMessage = isAuthError
    ? "You don't have permission to view or manage this roadmap."
    : error.message || "We encountered an unexpected error while trying to load this roadmap.";
      ...
      <div className="w-full max-w-md bg-dashboard-card border border-dashboard-cardBorder rounded-[var(--card-radius-lg)] p-8 shadow-sm space-y-5">
        ...
        <div>
          <h2 className="text-xl font-display font-bold text-text-primary">
            {isAuthError ? "Access Denied" : "Roadmap Error"}
          </h2>
