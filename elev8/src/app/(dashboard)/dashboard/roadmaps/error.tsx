...
export default function RoadmapLibraryError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("[RoadmapLibraryError] Error boundary caught error:", error);
  }, [error]);
  ...
  const displayMessage = isAuthError
    ? "You don't have permission to perform this action or view these roadmaps."
    : error.message || "An unexpected error occurred while loading your roadmaps. Please try again.";
