...
export default function NewInterviewError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("[NewInterviewError] Error boundary caught error:", error);
  }, [error]);
      ...
      <div className="w-full max-w-md bg-dashboard-card border border-dashboard-cardBorder rounded-[var(--card-radius-lg)] p-8 shadow-sm space-y-5">
        ...
        <div>
          ...
          <p className="text-sm font-sans text-text-secondary mt-2 leading-relaxed">
            {error.message || "An unexpected error occurred while preparing the interview generator."}
          </p>
        </div>
