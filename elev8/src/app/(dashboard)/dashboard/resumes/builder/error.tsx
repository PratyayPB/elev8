...
export default function ResumeBuilderError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("[ResumeBuilderError] Error boundary caught error:", error);
  }, [error]);
  ...
  const displayMessage = isAuthError
    ? "You do not have permission to view or manage these resumes."
    : error.message || "An unexpected error occurred while loading your resumes. Please try again.";
